...
public class StandardBullet : MonoBehaviourPun
{
    ...
    private void Start()
    {
        ...
        if (PhotonNetwork.IsMasterClient) StartCoroutine(DestroyAfterDelay(_bulletLife, this.gameObject)); 
    }

    private void OnCollisionEnter2D(Collision2D collision)
    {
        //if(photonView.IsMine) StartCoroutine(DestroyAfterDelay(0.005f, this.gameObject));
        if(PhotonNetwork.IsMasterClient) StartCoroutine(DestroyAfterDelay(0.005f, this.gameObject));
    }
